...
public class timeview {
	static timeController controller = new timeController();
	static timeModel model = new timeModel();
	...
	static void gugudan() {
		while (true) {
			try {
				...
				System.out.println("(메뉴 화면: 1 또는 0 입력)");
				String num = controller.getInput();

				// 구구단 계산하기
				if (num.equals("1") || num.equals("1") || Integer.parseInt(num) == 0) { // X 또는 0 입력 시 종료
