...
public class View {
		...
		while (true) { // 무한 루프로 프로그램 종료 선택까지 재실행
			...
			if (num.equals("1")) { // 1일시 구구단 프로그램 실행
				...
				if (isInteger2(number1, number2)) { // 입력받은 두 숫자가 정수 인지 판단후 올바르면 프로그램 실행
					...
					Calcultor.calculate(number1, sign, number2); // 입력받은 sign이 올바른 기호이면 입력한식과 값이 출력
				}
	...
	public static boolean isInteger2(String judgment0, String judgment2) { // 입력한 두 숫자가 정수인지 판단
		try {
